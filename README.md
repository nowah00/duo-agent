...
터미널에 `AI 협업 감시 시작` 메시지가 뜨면 준비 완료입니다.

### 2단계 — 작업 지시

새 터미널을 열고 하고 싶은 작업을 입력합니다.
...
에이전트의 작업 내용은 터미널에 실시간으로 출력됩니다.

### 4단계 — 결과 확인

- **`TASK_DONE.md`** — 완료 여부와 라운드 정보
- **`reviews/`** — 각 에이전트의 전체 출력 기록

### 다음 작업 지시

`TASK_DONE.md`를 삭제하지 않아도 됩니다. `kick` 명령이 자동으로 초기화합니다.

```bash
npm run kick "다음에 할 작업 내용"
```
