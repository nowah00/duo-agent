...
### 2026-04-27 (기능 추가·버그 수정·UI 정리)

**미리보기 탭 추가** (`src/main.js`)
- 내비게이션에 "미리보기" 탭(`#preview`) 추가
- `renderPreviewPage()`: `src/index.html`(게임·앱)을 iframe으로 대시보드 안에서 바로 실행
- "새 탭에서 열기 ↗", "ZIP 다운로드", "전체 화면" 버튼 제공
- `hashchange` 및 초기 URL 해시 파싱에 `#preview` 처리 추가
- 폴링 중 iframe 재생성으로 게임이 2초마다 새로고침되던 버그 수정 — 미리보기 탭에서는 `render()` 호출 생략

**`/game` 전용 라우트 추가** (`vite.config.js`)
- `GET /game` → `src/index.html` 서빙 (없으면 안내 페이지 반환)
- iframe `src="/"` 시 대시보드가 iframe 안에 중첩되던 버그 수정
- 미리보기 탭 내 "duo-agent 대시보드 →" 복귀 버튼 제거 — 탭으로 이동 가능

**템플릿·산출물 경계 명확화** (`AGENTS.md`, `CLAUDE.md`, `vite.config.js`)
- `index.html`(루트), `src/main.js` = 템플릿 — 에이전트 수정 금지 명시
- `src/index.html` = 산출물 진입점으로 명시
- `transformIndexHtml` 복귀 버튼 주입 대상에서 `/` 제외
- ZIP 다운로드에서 `src/main.js` 제외

**초기화 시 대시보드 UI 함께 초기화** (`src/main.js`)
- `/api/reset` 성공 후 킥 폼·피드백 초안·열린 리뷰 목록·에러 메시지 일괄 초기화

**불필요 기능 제거** (`src/main.js`)
- **진행 중 피드백 패널** 제거 — 타이밍 문제로 실용성 낮고 Stop 후 재요청으로 대체 가능
- **검토 결과 프롬프트 탭** 제거 — 에이전트 내부 프롬프트는 사용자 관심 밖
- **현재 작업 요약 메트릭** 제거 — 하단 "현재 요청" 블록과 중복; 메트릭 그리드 3칸→2칸
- **최근 결과 요약 패널** 간소화 — decision·nextAction 텍스트 제거, 에이전트 요약 + 변경 파일 수만 유지

---

### 2026-04-26 23:22 (기능 추가 — 프로젝트 관리: ZIP 다운로드, 초기화 모달, 대시보드 복귀 버튼)

**프로젝트 이름 입력** (`src/main.js`)
- 작업 시작 폼에 프로젝트 이름 입력 필드 추가 — 초기화 전까지 유지되며 `watcher/project.txt`에 저장

**ZIP 다운로드** (`vite.config.js`)
- `/api/download-src` 엔드포인트 추가 — `src/` + `index.html`을 `프로젝트명.zip`으로 내보내기

**프로젝트 초기화** (`vite.config.js`, `src/main.js`)
- `/api/reset` 엔드포인트 추가 — 모달 확인 후 `src/` 생성 파일·작업 내역 삭제 및 git으로 원본 복원
- 초기화 확인 모달: 오버레이 클릭·Esc 키·취소 버튼으로 닫기, 진행 중 버튼 비활성화

**대시보드 항상 접근 가능** (`vite.config.js`)
- `/_dashboard` 전용 라우트 추가 — `index.html`이 게임 화면으로 교체돼도 대시보드 접근 유지
- `transformIndexHtml`으로 게임 화면 우측 하단에 "duo-agent 대시보드 →" 고정 버튼 자동 주입

---

### 2026-04-24 17:00 (문서 — 개발 로그 항목에 커밋 시간 추가)

**개발 로그 가독성 개선** (`README.md`)
- 기존 날짜만 표시되던 로그 헤더에 커밋 시간 반영 (`2026-04-24` → `2026-04-24 16:46`)
- git log 커밋 타임스탬프 기준으로 모든 항목 일괄 갱신

---

### 2026-04-24 16:46 (정리 — 노후화 코드 최신화 및 불필요 규칙 제거)
...
### 2026-04-24 16:35 (UI 개편 — 상태 설명 한국화, 오류 번역, 히스토리 세션 배지)
...
### 2026-04-24 14:23 (기능 추가 — 프롬프트·결과 내역 매칭)
...
### 2026-04-24 14:06 (기능 추가 — 피드백 채널 및 라운드 요약)
...
### 2026-04-24 13:10 (리팩토링 — 에이전트 추상화 및 안정화)
...
### 2026-04-24 13:04 (버그 수정 및 기능 추가)
...
### 2026-04-23 16:25 ~ 2026-04-24 00:13
